...
async fn run_data_sync_demo() {
    ...
    let height_a = node_a.storage.get_best_height().unwrap();
    ...
    let height_b = node_b.storage.get_best_height().unwrap();
    ...
    let new_height_a = node_a.storage.get_best_height().unwrap();
    ...
    let height_c = node_c.storage.get_best_height().unwrap();
    ...
    sync_nodes(&node_a, &node_b).await;
    let final_height_b = node_b.storage.get_best_height().unwrap();
    ...
    let final_height_a = node_a.storage.get_best_height().unwrap();
    let final_height_b = node_b.storage.get_best_height().unwrap();
    let final_height_c = node_c.storage.get_best_height().unwrap();
...
async fn create_blockchain(node: &Node, block_count: u64) {
    let mut previous_hash = node.storage.get_block_by_height(0).unwrap().hash();
...
async fn create_blocks_with_txs(node: &Node, block_count: u64, accounts: &[Address]) {
    let current_height = node.storage.get_best_height().unwrap();
...
async fn sync_nodes(source: &Node, target: &Node) {
    ...
    let target_best_height = target.storage.get_best_height().unwrap();
