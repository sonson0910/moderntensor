...
fn register_dispute_submit(ctx: &DisputeRpcContext, io: &mut IoHandler) {
    ...
    io.add_method("dispute_submit", move |params: Params| {
        let dm = dm.clone();
        async move {
...
fn register_dispute_get_status(ctx: &DisputeRpcContext, io: &mut IoHandler) {
    ...
    io.add_method("dispute_getStatus", move |params: Params| {
        let dm = dm.clone();
        async move {
            ...
            match dm.get_dispute_status(&request_id).await {
                Some(status) => {
...
fn register_dispute_stats(ctx: &DisputeRpcContext, io: &mut IoHandler) {
    ...
    io.add_method("dispute_stats", move |_params: Params| {
        let dm = dm.clone();
        async move {
            let pending = dm.pending_count().await;
            let active = dm.active_dispute_count().await;
