...
pub fn register_staking_methods(
    ...
    io.add_method("staking_stake", move |params: Params| {
        let db = db.clone();
        async move {
    ...
    let db = metagraph_db.clone();
    io.add_method("staking_unstake", move |params: Params| {
        let db = db.clone();
        async move {
            let parsed: Vec<String> = params.parse()?;
            if parsed.len() < 4 {
    ...
    let db = metagraph_db.clone();
    io.add_method("staking_delegate", move |params: Params| {
        let db = db.clone();
        async move {
            let parsed: Vec<String> = params.parse()?;
            if parsed.len() < 5 {
    ...
    let db = metagraph_db.clone();
    io.add_method("staking_undelegate", move |params: Params| {
        let db = db.clone();
        async move {
            let parsed: Vec<String> = params.parse()?;
            if parsed.len() < 3 {
    ...
    io.add_method("staking_getMinimums", move |_params: Params| {
        async move {
            Ok(serde_json::json!({
    ...
    let db = metagraph_db.clone();
    io.add_method("staking_getStakeForPair", move |params: Params| {
        let db = db.clone();
        async move {
            let parsed: Vec<String> = params.parse()?;
            if parsed.len() < 2 {
    ...
    let db = metagraph_db.clone();
    io.add_method("staking_getAllStakesForColdkey", move |params: Params| {
        let db = db.clone();
        async move {
            let parsed: Vec<String> = params.parse()?;
            if parsed.is_empty() {
    ...
    let db = metagraph_db.clone();
    io.add_method("staking_getDelegates", move |_params: Params| {
        let db = db.clone();
        async move {
            let stakes = db.get_all_stakes()
                .map_err(|e| internal_error(&e.to_string()))?;
