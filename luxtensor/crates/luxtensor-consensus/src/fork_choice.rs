...
impl ForkChoice {
    ...
    pub fn recompute_scores(&self) {
        ...
        // Clear existing scores except genesis
        scores.clear();
        scores.insert(self.genesis_hash, 0);
        
        // Recompute scores by traversing from genesis
        // This is a simplified version - a full implementation would use
        // a topological sort or BFS from genesis
        for (hash, block) in blocks.iter() {
            let parent_hash = block.header().previous_hash;
            let parent_score = if parent_hash == [0u8; 32] {
                0
            } else {
                scores.get(&parent_hash).copied().unwrap_or(0)
            };
            scores.insert(*hash, parent_score + 1);
        }
    ...
    pub fn prune(&self, keep_depth: u64) -> Result<usize, ConsensusError> {
        ...
        // Find canonical block hashes to keep
        let mut keep_hashes: Vec<Hash> = canonical_chain
            .iter()
        ...
        // Also keep recent fork blocks
        {
            ...
            for block in blocks.values() {
                if block.height() >= prune_before_height {
                    keep_hashes.push(block.hash());
                }
            }
        }
        
        // Remove blocks not in keep list
        let mut blocks = self.blocks.write();
        let mut scores = self.scores.write();
        let initial_count = blocks.len();
        
        blocks.retain(|hash, _| keep_hashes.contains(hash));
        scores.retain(|hash, _| keep_hashes.contains(hash));
