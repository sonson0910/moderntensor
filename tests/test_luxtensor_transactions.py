...
class TestLuxtensorTransaction:
    ...
    def test_signing_message_generation(self):
        ...
        # 8 bytes (chain_id) + 8 bytes (nonce) + 20 bytes (from) + 20 bytes (to) + 16 bytes (value) + 8 bytes (gas_price) + 8 bytes (gas_limit) + data
        expected_min_length = 8 + 8 + 20 + 20 + 16 + 8 + 8 + 2
