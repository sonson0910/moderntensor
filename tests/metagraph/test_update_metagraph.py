...
def new_datum(miner_data_for_update):
    ...
    api_endpoint_bytes = (
        miner_data_for_update["api_endpoint"].encode("utf-8")
        if miner_data_for_update["api_endpoint"]
        else None
    )
    ...
    datum = MinerDatum(
        ...
        scaled_last_performance=int(
            miner_data_for_update["last_performance"] * divisor
        ),
...
def test_update_datum(chain_context_fixture, mock_utxo, new_datum, hotkey_skey_fixture):
    ...
    logger.info(
        f"Attempting to update UTxO {mock_utxo.input} at script {script_hash} on {network}..."
    )
