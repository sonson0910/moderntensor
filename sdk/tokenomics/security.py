...
class AuditLogger:
    ...
    def log_event(
        ...
        event = {
            ...
            'action': action,
            'details': details or {},
            'hash': self._calculate_event_hash()
        }
        
        self.audit_log.append(event)
    
    def _calculate_event_hash(self) -> str:
        """Calculate hash of current audit log state."""
        if not self.audit_log:
            return hashlib.sha256(b'').hexdigest()
        
        last_hash = self.audit_log[-1].get('hash', '')
        data = f"{last_hash}{time.time()}".encode()
    ...
    def verify_integrity(self) -> bool:
        ...
        for i, event in enumerate(self.audit_log):
            ...
            # In production, would verify event hash against prev_hash
